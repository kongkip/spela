...
class AdditiveNoise(tf.keras.layers.Layer):
    ...
    ### `AdditiveNoise`
    ```python
    spela.augmentation.AdditiveNoise(power=0.1, random_gain=False, noise_type='white', **kwargs)
    ```
    ...
    * noise_type; str,
    ...
    def call(self, x, **kwargs):
        if self.random_gain:
            noise_x = x + tf.keras.backend.random_normal(shape=tf.keras.backend.shape(x),
                                                         mean=0.,
